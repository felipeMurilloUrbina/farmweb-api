...
namespace Farmacia.POS.WebApi.Controllers
{
    ...
    {
        readonly ServicioCategoria _servicio = null;
        ...
        public async Task<IHttpActionResult> Post(CategoriaDto categorias)
        {
            if (category == null)
        ...
        public async Task<IHttpActionResult> Put(CategoriaDto categorias)
        {
            if (category == null)
                return BadRequest(String.Format(Resources.RequestEmpty, _element));
            var principal = RequestContext.Principal as ClaimsPrincipal;
