...
namespace Farmacia.POS.WebApi.Controllers
{
    ...
    {
        readonly ServicioCliente _servicio = null;
        Contexto _contexto = null;
        string _element = "Cliente";
        ...
        public async Task<IHttpActionResult> Post(ClienteDto clientes)
        {
            if (category == null)
        ...
        public async Task<IHttpActionResult> Put(ClienteDto clientes)
        {
            if (category == null)
                return BadRequest(String.Format(Resources.RequestEmpty, _element));
            var principal = RequestContext.Principal as ClaimsPrincipal;
